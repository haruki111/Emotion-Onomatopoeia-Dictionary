...
class EmotionObject {
    ...
    getOnomatopoeiaWords() {
        ...
        for (let i = 0; i < this.onomatopoeia.length; i++) {
            let CurrentWord = new Word(this.onomatopoeia[i], dictionary[this.onomatopoeia[i]], pictureDictionary[this.onomatopoeia[i]]);
    ...
    getHtmlCategoryString() {
        ...
        let category =
            `
            <a class="category_item bg_${this.color}" href = "#${this.emotion}">
                <div class="category_title">${this.emotion.toUpperCase()}</div>
                <div class="category_img">
                    <img
                    src="${onomatopoeiaWords[0].pictureUrl}"
                    />
                </div>
            </a>
            `;
    ...
    getHtmlContainerString() {
        ...
        let container =
            `
                <div id = "${this.emotion}" class="container_item inner bg_${this.color}">
                    <h3 class="container_emotion">${this.emotion.toUpperCase()}</h3>
                    <div class="container_description">${this.description}</div>
                    <div class="container_card_items">
            `;
        ...
        for (let i = 0; i < onomatopoeiaWords.length; i++) {
            ...
            container +=
                ...
                <div class="container_card_item">
                    <div class="container_card_item_body">
                        <div class="container_card_title">${currentWord.word}</div>
                        <div class="container_card_defintion">
                            ${currentWord.defintion}
                        </div>
                    </div>
                    <div class="container_card_item_img">
                        <img src="${currentWord.pictureUrl}">
                    </div>
                 </div>
                `;
        }
